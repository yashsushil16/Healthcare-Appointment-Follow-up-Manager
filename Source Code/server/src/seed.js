...
async function seed(force = false) {
  ...
  if (doctorCount > 0 && !force) {
    console.log(`[Seed] Database already contains ${doctorCount} doctor profiles. Auto-seed skipped.`);
    return;
  }
  ...
  // 1. Create Admin Account if not existing
  let admin = await prisma.user.findUnique({ where: { email: 'admin@drpatho.com' } });
  if (!admin) {
  ...
  // 2. Create Demo Patient Account if not existing
  let patient = await prisma.user.findUnique({ where: { email: 'patient@drpatho.com' } });
  if (!patient) {
